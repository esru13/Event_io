...
const FAQItem = ({ question, answer }) => {
  ...
  return (
    <div className="border border-white rounded-lg p-4 pl-8 mb-2 mx-auto w-full max-w-6xl bg-white">
      ...
      >
        <span>{question}</span>
        <span className="text-black font-bold text-3xl">{isOpen ? "-" : "+"}</span>
      </button>
      ...
      >
        <p className="mt-4 text-black font-poppins italic">{answer}</p>
      </div>
    </div>
  );
};
...
const EventTicketFAQ = () => {
  return (
    <div className="w-full min-h-screen bg-blue-50 px-6 py-20">
